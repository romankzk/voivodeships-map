...
export function usePeriodData(periodId: string): { data: PeriodData | null; isLoading: boolean } {
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetch(`./data/${periodConfig.areasFile}.geojson`, { signal }).then(r => r.json()),
            fetch(`./data/${periodConfig.bordersFile}.geojson`, { signal }).then(r => r.json()),
            fetch(`./data/${periodConfig.pointsFile}.geojson`, { signal }).then(r => r.json()),
        ])
